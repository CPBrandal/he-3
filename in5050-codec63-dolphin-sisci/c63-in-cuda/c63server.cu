...
switch(cmd) {
  case CMD_HELLO:
      ...
      // Prepare response message in uppercase
      strcpy((char*)local_seg->message_buffer, "HELLO FROM SERVER");
      local_seg->packet.data_size = strlen("HELLO FROM SERVER") + 1;
      ...
      remote_seg->packet.cmd = CMD_HELLO_ACK;
      SCIFlush(NULL, NO_FLAGS);
